    ...
    with current_app.app_context():
        ...
        if not file_metadata:
            self.update_state(
                ...
                meta={
                    "exc_type": "ValueError",
                    "exc_message": f"File metadata not found in Redis for file_id: {file_id} (possibly expired)",
                },
            )
            return {
                "error": f"File metadata not found for file_id: {file_id}",
                "status": "FAILURE_METADATA_NOT_FOUND",
            }

        redis_client.hmset(
            meta_key, {"status": "PROCESSING", "celery_task_id": self.request.id}
        )
        ...
        if "error" in influence_graph_data:
            errors_list.append(
                f"Influence graph error: {influence_graph_data['error']}"
            )
