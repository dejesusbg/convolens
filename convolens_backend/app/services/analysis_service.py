...
def get_speaker_from_line_txt(line):
    ...
    if ":" in line:
        ...
        if len(possible_speaker) < 50 and not possible_speaker.isdigit(): # Basic filter
...
def get_speaker_from_csv_row(row, fieldnames):
    ...
    if not actual_speaker_col and isinstance(row, list) and row:
        ...
        if len(possible_speaker) < 50 and not possible_speaker.isdigit() and ":" not in possible_speaker:
            return possible_speaker
    elif isinstance(row, dict) and not actual_speaker_col: # If it's a dict but our named cols weren't found
...
def calculate_interaction_frequency(filepath):
        ...
        if filename.endswith('.txt'):
            ...
            with open(filepath, 'r', encoding='utf-8') as f:
                ...
                if isinstance(data, list):
                    messages = data
                elif isinstance(data, dict) and 'transcript' in data and isinstance(data['transcript'], list):
                    messages = data['transcript']
                elif isinstance(data, dict) and 'log' in data and 'messages' in data['log'] and isinstance(data['log']['messages'], list):
    ...
    links = [{"source": source, "target": target, "value": value} for (source, target), value in interactions.items()]
...
def identify_speakers_from_csv(filepath):
    ...
    try:
        with open(filepath, 'r', encoding='utf-8', newline='') as f:
            temp_f_for_sniffer = open(filepath, 'r', encoding='utf-8', newline='')
            try:
                has_header = csv.Sniffer().has_header(temp_f_for_sniffer.read(2048)) # Read more bytes for sniffer
            except csv.Error: # Sniffer can fail on perfectly valid CSVs if it's confused
...
def extract_speaker_statistics(filepath):
    ...
    if not speaker_counts:
        return {"error": "Could not identify any speakers or file might be empty/misformatted for speakers."}
...
def analyze_emotions_with_text2emotion(text_content_list):
    ...
    for text in text_content_list:
        if not text or not isinstance(text, str) or text.isspace():
            results.append({"text": text, "emotions": {}, "error": "Empty or invalid input text"})
            continue
        try:
            emotion_scores = te.get_emotion(text)
            results.append({"text": text, "emotions": emotion_scores})
        except Exception as e:
            results.append({"text": text, "emotions": {}, "error": f"Error during emotion analysis: {str(e)}"})
        ...
        if filename.endswith('.txt'):
            with open(filepath, 'r', encoding='utf-8') as f:
                for line in f:
                    ...
                    if match and match.group(2).strip(): # Ensure text part is not empty
                        texts.append(match.group(2).strip())
                    elif not match and line_content: # No speaker tag, assume whole line is text
            ...
            with open(filepath, 'r', encoding='utf-8') as f:
                data = json.load(f)
                messages_data = []
                if isinstance(data, list): messages_data = data
                elif isinstance(data, dict) and 'transcript' in data and isinstance(data['transcript'], list): messages_data = data['transcript']
                elif isinstance(data, dict) and 'log' in data and 'messages' in data['log'] and isinstance(data['log']['messages'], list): messages_data = data['log']['messages']

                for item in messages_data:
                    if isinstance(item, dict):
                        for key in ['text', 'message', 'line', 'content', 'utterance', 'msg']: # Added 'msg'
                            if key in item and isinstance(item[key], str) and item[key].strip():
                                texts.append(item[key].strip())
                                break
                        # Watson specific path for some logs
                        elif 'response' in item and isinstance(item['response'], dict) and                              'output' in item['response'] and isinstance(item['response']['output'], dict) and                              isinstance(item['response']['output'].get('generic'), list) and                              item['response']['output']['generic'] and                              isinstance(item['response']['output']['generic'][0], dict) and                              item['response']['output']['generic'][0].get('text'):
                             texts.append(item['response']['output']['generic'][0]['text'].strip())


        elif filename.endswith('.csv'):
            with open(filepath, 'r', encoding='utf-8', newline='') as f:
                temp_f_for_sniffer_text = open(filepath, 'r', encoding='utf-8', newline='')
                try:
                    has_header_text = csv.Sniffer().has_header(temp_f_for_sniffer_text.read(2048))
                except csv.Error:
                ...
                if has_header_text:
                    reader = csv.DictReader(f)
                    text_col_names = ['text', 'message', 'content', 'utterance', 'line', 'transcript', 'msg']
                        ...
                        for name_actual in reader.fieldnames:
                            for name_potential in text_col_names:
                                if name_potential.lower() == name_actual.lower():
                                    ...
                                    break
                            if actual_text_col: break

                    if actual_text_col:
                        for row in reader:
                            if row.get(actual_text_col) and isinstance(row[actual_text_col], str) and row[actual_text_col].strip():
                    ...
                    for row in reader:
                        ...
                        text_candidate = ""
                        if len(row) > 1: text_candidate = row[-1].strip() # last column often text
                        elif row : text_candidate = row[0].strip()

                        if text_candidate: texts.append(text_candidate)
    except Exception as e:
...
ETHOS_LEXICON = [
    ...
    "expert", "expertise", "authority", "authoritative", "credentials", "experience", "experienced",
    "proven", "track record", "reliable", "trustworthy", "honest", "integrity", "sincere",
    "research shows", "studies indicate", "according to experts", "dr.", "professor",
    "we believe", "our commitment", "our values", "ethically", "responsible"
]

PATHOS_LEXICON = [
    # Emotions, Values, Stories
    "imagine", "feel", "feeling", "heart", "soul", "spirit", "passion", "passionate",
    "hope", "dream", "aspire", "desire", "yearn",
    "joy", "happy", "delight", "pleasure", "wonderful", "amazing", "fantastic", "miracle",
    "sad", "sorrow", "pain", "suffering", "heartbreaking", "tragic", "unfortunate",
    "fear", "afraid", "danger", "risk", "threat", "anxiety", "worry",
    "anger", "outrage", "frustration", "injustice", "unfair",
    "love", "compassion", "empathy", "care", "kindness", "sympathy",
    "urgent", "critical", "immediate", "now", "crisis", "must act",
    "story", "tale", "narrative", "journey", "vulnerable", "struggle", "victory",
    "our children", "our future", "community", "family", "shared values", "common good"
]

LOGOS_LEXICON = [
    # Logic, Reason, Evidence, Data
    "logic", "logical", "reason", "rational", "rationale", "sound argument",
    "evidence", "proof", "data", "statistics", "facts", "figures", "numbers", "chart", "graph",
    "analysis", "analyze", "analytical", "study", "research", "findings",
    "because", "therefore", "consequently", "as a result", "thus", "hence", "ergo",
    "if...then", "since", "given that", "it follows that",
    "clear", "clearly", "obvious", "evidently", "plainly",
    "demonstrates", "shows", "indicates", "points to", "verifies", "confirms",
    "hypothesis", "theory", "principle", "premise", "conclusion",
    "compare", "contrast", "differentiate", "classify", "organize", "systematic"
]
...
def calculate_persuasion_scores_heuristic(text_content_list):
    ...
    ethos_patterns = [re.compile(r'\b' + re.escape(word) + r'\b', re.IGNORECASE) for word in ETHOS_LEXICON]
    pathos_patterns = [re.compile(r'\b' + re.escape(word) + r'\b', re.IGNORECASE) for word in PATHOS_LEXICON]
    logos_patterns = [re.compile(r'\b' + re.escape(word) + r'\b', re.IGNORECASE) for word in LOGOS_LEXICON]

    for text in text_content_list:
        if not text or not isinstance(text, str) or text.isspace():
            results.append({
                "text": text,
                "ethos_score": 0, "pathos_score": 0, "logos_score": 0,
                "ethos_matches": [], "pathos_matches": [], "logos_matches": [],
...
AD_HOMINEM_KEYWORDS = [
    "idiot", "stupid", "moron", "ignorant", "fool", "jerk", "loser", "naive",
    "you are dumb", "you're a joke", "he is a liar", "she is incompetent", "they are clueless",
...
FALSE_DICHOTOMY_PHRASES = [
    ...
    "no middle ground", "black and white thinking"
...
GUILT_TRIPPING_KEYWORDS = [
    "if you cared", "if you loved me", "you would understand if", "don't you feel bad",
    "after all I've done for you", "I sacrificed so much", "you owe me",
    "making me feel guilty", "you always make me" # Accusation of it
]

# Keywords for very basic Gaslighting indicators
GASLIGHTING_KEYWORDS = [
    "you're imagining things", "that never happened", "you are crazy", "you're being irrational",
    "you're too sensitive", "don't be so dramatic", "I never said that", "you're misremembering",
...
def detect_fallacies_and_manipulation_heuristic(text_content_list):
    ...
    ad_hominem_patterns = [re.compile(r'\b' + re.escape(kw) + r'\b', re.IGNORECASE) for kw in AD_HOMINEM_KEYWORDS]
    ...
    guilt_tripping_patterns = [re.compile(r'\b' + re.escape(kw) + r'\b', re.IGNORECASE) for kw in GUILT_TRIPPING_KEYWORDS]
    gaslighting_patterns = [re.compile(r'\b' + re.escape(kw) + r'\b', re.IGNORECASE) for kw in GASLIGHTING_KEYWORDS]

    for text in text_content_list:
        if not text or not isinstance(text, str) or text.isspace():
            results.append({
                "text": text, "detected_fallacies": [], "detected_manipulations": [],
                "error": "Empty or invalid input text"
            })
            continue
        ...
        for pattern in ad_hominem_patterns:
            matches = pattern.findall(text)
            if matches:
                detected_fallacies.append({"type": "Ad Hominem", "keywords_matched": list(set(matches))})
        ...
        for phrase in FALSE_DICHOTOMY_PHRASES:
            if phrase.lower() in text_lower: # Simple substring check for phrases
                ...
                if phrase == "either...or":
                    if "either " in text_lower and " or " in text_lower.split("either ")[-1]:
                         detected_fallacies.append({"type": "False Dichotomy", "phrase_matched": phrase})
                         break
                else:
                    detected_fallacies.append({"type": "False Dichotomy", "phrase_matched": phrase})
                    break
        ...
        for pattern in guilt_tripping_patterns:
            matches = pattern.findall(text)
            if matches:
                detected_manipulations.append({"type": "Guilt Tripping", "keywords_matched": list(set(matches))})
                break
        ...
        for pattern in gaslighting_patterns:
            matches = pattern.findall(text)
            if matches:
                detected_manipulations.append({"type": "Gaslighting (potential)", "keywords_matched": list(set(matches))})
                break
