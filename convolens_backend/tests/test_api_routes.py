...
from app.models import Conversation, ConversationStatus, db # Import models
...
def test_upload_file_success(client, clean_db):
    ...
    with client.application.app_context():
        ...
        assert convo.status == ConversationStatus.UPLOADED
...
def test_list_conversations_with_data_and_filter(client, clean_db):
    ...
    with client.application.app_context():
        convo1 = Conversation(file_id="file1.txt", original_filename="file1.txt", status=ConversationStatus.COMPLETED, language="en")
        convo2 = Conversation(file_id="file2.txt", original_filename="file2.txt", status=ConversationStatus.PROCESSING, language="es")
...
def test_start_analysis_task_success(client, clean_db, mock_celery_task):
    ...
    with client.application.app_context():
        convo = Conversation(file_id=file_id_for_analysis, original_filename="analysis_test_file.txt", status=ConversationStatus.UPLOADED)
    ...
    with client.application.app_context():
        updated_convo = Conversation.query.filter_by(file_id=file_id_for_analysis).first()
        assert updated_convo.status == ConversationStatus.PENDING_ANALYSIS
