import pytest
from app.services.analysis_service import extract_speaker_statistics, calculate_persuasion_scores_heuristic
import os
import tempfile
...
def test_extract_speaker_statistics_txt():
    ...
    test_content = "Speaker A: Hello world\nSpeaker B: Hi there\nSpeaker A: How are you?"
...
def test_persuasion_scoring_basic():
    ...
    assert "logical" in results[0]["logos_matches"] or "evidence" in results[0]["logos_matches"]

    # Check second text for pathos
    assert results[1]["logos_score"] == 0
    assert results[1]["pathos_score"] > 0
    assert results[1]["ethos_score"] == 0
    assert "passionate" in results[1]["pathos_matches"] or "children" in results[1]["pathos_matches"]

    # Check third text for ethos
    assert results[2]["logos_score"] == 0
    assert results[2]["pathos_score"] == 0
    assert results[2]["ethos_score"] > 0
    assert "expert" in results[2]["ethos_matches"] or "trustworthy" in results[2]["ethos_matches"]
