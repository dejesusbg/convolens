...
def test_upload_file_success(client, redis_client, mocker): # Added redis_client and mocker
    ...
    mock_expire.assert_called_once_with(expected_redis_key, client.application.config["REDIS_CACHE_TTL_SECONDS"])


def test_upload_file_invalid_type(client, redis_client): # Replaced clean_db with redis_client
...
def test_upload_file_unsupported_language(client, redis_client): # Replaced clean_db with redis_client
    data = {
...
def test_list_conversations_with_data_and_filter(client, redis_client): # Replaced clean_db
    # Setup: Populate Redis with mock data
    redis_client.hmset("filemeta:file1.txt", {
        "file_id": "file1.txt",
    ...
    redis_client.hmset("filemeta:file2.txt", {
        "file_id": "file2.txt",
    ...
    conv_data_file1 = next(c for c in json_data["conversations"] if c["file_id"] == "file1.txt")
    ...
    # Pagination was removed, so no pagination tests here.
    # If basic list slicing were implemented, add tests for that.
...
def test_start_analysis_task_success(client, redis_client, mock_celery_task, mocker): # Added redis_client, mocker
    ...
    mock_redis_hmset.assert_called_once_with(meta_key, {"celery_task_id": task_id, "status": "PENDING_ANALYSIS"})
    ...
    mock_redis_set.assert_called_once_with(expected_task_to_fileid_key, file_id_for_analysis, ex=ttl)

    # 3. Expire on meta_key (called in the route)
    mock_redis_expire.assert_called_once_with(meta_key, ttl)


def test_start_analysis_task_file_not_found(client, redis_client, mock_celery_task): # Replaced clean_db
    ...
    json_data = response.get_json()
    assert "File metadata for file_id: non_existent_file.txt not found" in json_data["description"]
...
def test_get_conversation_details_success(client, redis_client):
    ...
    assert "analysis_result_summary_url" in json_data # Check for presence, content depends on url_for
...
def test_get_task_status(client, redis_client, mocker):
    ...
    redis_client.hmset(f"filemeta:{file_id}", {"status": "PROCESSING", "celery_task_id": task_id})
    ...
    mocker.patch("celery.result.AsyncResult", return_value=mock_async_result) # Re-patch for new state
...
def test_get_task_result_success(client, redis_client, mocker):
    ...
    # Populate Redis
    redis_client.set(f"task_to_fileid:{task_id}", file_id)
    redis_client.hmset(f"filemeta:{file_id}", {"status": "COMPLETED", "celery_task_id": task_id})
...
def test_get_task_result_not_found(client, redis_client):
    ...
    assert "Mapping for task_id unmapped_task to file_id not found" in response_no_map.get_json()["description"]
    ...
    assert "File metadata for file_id file_no_meta.txt" in response_no_meta.get_json()["description"]
    ...
    redis_client.hmset(f"filemeta:{file_id_no_result}", {"status": "COMPLETED"}) # Status is completed
    ...
    assert f"Analysis result data for file_id {file_id_no_result}" in response_no_result_data.get_json()["description"]
...
def test_get_specific_analysis_result_success(client, redis_client):
    ...
    assert json_data["analysis_type_found"] == "emotion_analysis" # Assuming this is the key in stored data
...
def test_get_specific_analysis_result_not_found(client, redis_client):
    ...
    response_no_meta = client.get(f"/api/conversations/{file_id}/results/emotion_analysis")
    assert response_no_meta.status_code == 404
    assert "Conversation metadata not found" in response_no_meta.get_json()["description"]

    # Case 2: analysisresult (main result file) missing
    redis_client.hmset(f"filemeta:{file_id}", {"status": "COMPLETED"}) # Meta exists
    response_no_main_result = client.get(f"/api/conversations/{file_id}/results/emotion_analysis")
    assert response_no_main_result.status_code == 404
    assert "Analysis result data not found in Redis" in response_no_main_result.get_json()["description"]
    redis_client.delete(f"filemeta:{file_id}") # clean up for next case

    # Case 3: Specific analysis_type key missing in results
    redis_client.hmset(f"filemeta:{file_id}", {"status": "COMPLETED"})
    mock_partial_results = {"results": {"persuasion_analysis": {"effectiveness": "low"}}, "errors": []}
    redis_client.set(f"analysisresult:{file_id}", json.dumps(mock_partial_results))
    response_no_type = client.get(f"/api/conversations/{file_id}/results/emotion_analysis") # Requesting emotion
    assert response_no_type.status_code == 404
    assert "Invalid analysis type 'emotion_analysis'" in response_no_type.get_json()["description"]

    # Case 4: Analysis not yet complete
    redis_client.hmset(f"filemeta:{file_id}", {"status": "PROCESSING"}) # Status is processing
    # result data may or may not exist yet
    response_not_complete = client.get(f"/api/conversations/{file_id}/results/emotion_analysis")
    assert response_not_complete.status_code == 409 # Conflict / Not ready
    assert "Analysis not yet complete or failed" in response_not_complete.get_json()["description"]
